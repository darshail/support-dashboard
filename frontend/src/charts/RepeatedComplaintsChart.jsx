...
export default function RepeatedComplaintsChart({
  data = [],
  activeKey = null,
  onSelect,
}) {
      ...
      >
          ...
          content={
            <ChartTooltip
              valueFormatter={(v, name) => {
                if (name === 'Avg rating' && v > 0) return `${Number(v).toFixed(2)} / 5`
                if (name === 'Avg rating') return 'No ratings'
        ...
        <Bar
          ...
          onClick={(bar) => {
            ...
            onSelect({ key: bar.key, name: bar.fullName ?? bar.name })
          }}
        >
          {data.map((entry) => (
            <Cell
